            ...
            for start in range(0, len(rows), export_config.batch_size):
                ...
                if export_config.output_format == "jsonl":
                    for index, row in enumerate(batch_rows):
                        probe_id = row.get("probe_id") or row.get("aux_id") or row.get("sidecar_id")
                        payload = {
                            "probe_id": probe_id,
                    ...
                    probe_ids.extend((row.get("probe_id") or row.get("aux_id") or row.get("sidecar_id")) for row in batch_rows)
