...
- validation이 있는 run에서는 `best_model.pt`가 계속 policy-best이며, 선택 기준은 `val.target_move_nll` 최소입니다. validation이 없는 run에서는 backward compatibility를 위해 마지막 epoch checkpoint를 `best_model.pt`로도 남깁니다.
- 학습은 `best_model.pt`, `best_policy_model.pt`, `best_grounding_model.pt`, `last_model.pt`, `selection_summary.*`, `pareto_epochs.json`, `train_metrics.json`, `run_metadata.json`, config 사본을 남깁니다.
- 평가는 `val/test` split별 `eval_report.*`, `eval_failures.jsonl`, 상위 `eval_summary.*`를 남깁니다. `eval_report.*`와 `eval_summary.*`에는 legal-conditioned move quality와 raw action-space honesty diagnostics가 함께 기록됩니다.
- `python -m modalchess.eval.aggregate_week1 --input-root outputs/week1`로 run별 표와 subset 표를 다시 만들 수 있습니다.

### move-quality 지표는 두 층으로 본다

- legal-conditioned metrics: 기존 `top_k`, `top_k_move_accuracy`, `target_move_nll`은 합법 수 집합 안에서의 랭킹 품질이다.
- raw honesty diagnostics: `illegal_top_1_rate`, `raw_top_1_is_legal_rate`, `legal_mass`, `illegal_mass`, `raw_target_move_nll`, `raw_top_k`는 legality masking 이전 전체 factorized action space에서 본 정직성 진단이다.
- 이번 구현에서는 raw action-space top-1/top-3/top-5도 exact하게 계산한다. 따라서 illegal top-1과 legal mass를 legal-conditioned accuracy와 직접 병기할 수 있다.

### checkpoint selection 규약

- validation이 있는 run에서는 `best_model.pt`는 backward compatibility를 위해 계속 policy-best다.
- `best_policy_model.pt`도 같은 의미이며, 기준은 `val.target_move_nll` 최소다.
- `best_grounding_model.pt`는 `mean(val.occupied_square_accuracy, val.piece_macro_f1, val.legality_average_precision)` 최대 epoch다.
- grounding selection에 필요한 세 metric 중 하나라도 validation output에 없으면 partial grounding score를 만들지 않고 명시적으로 실패시킨다.
- `pareto_epochs.json`은 `val.target_move_nll` 최소화, `val.occupied_square_accuracy`/`val.piece_macro_f1`/`val.legality_average_precision` 최대화 축에서 non-dominated epoch를 남긴다.
