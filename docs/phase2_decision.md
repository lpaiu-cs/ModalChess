...
**1) 공간 단독 vs FEN → FEN 우위 (P1, 확정 반증).** LM은 FEN 텍스트를 native하게 읽음.
pooled 공간 토큰은 판독에서 열세. 사전학습 인코더는 원시 plane과 **동률**(raw 판독엔 무이득).

**2) FEN+시각 vs FEN 단독 → 시각이 보강 (P1b, 확정).** hybrid 0.886 > fen_soft 0.866.
...
파생 상태(체크·핀)를 텍스트에 더하는 것"에 정확히 한정되며, 그것은 진짜다.**
